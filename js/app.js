...
(function () {
  ...
  function isBudgetTx(t) {
    ...
    return BUDGET_SUBS.has(t.subcategory) || BUDGET_CATS.has(t.category) ||
           t.source === 'sms' || t.source === 'kb_card';
  }
  ...
  /* ============================================================
     ...
     ============================================================ */
  const CARD_SRC = new Set(['samsung_card', 'hyundai_card', 'kb_card']);
  ...
  async function dedupeSmsVsCard() {
    ...
    for (const s of sms) {
      ...
      const hit = cards.find(c => {
        if (used.has(c.id) || dayGap(c.tx_date, s.tx_date) > 3) return false;
        const sameMer = merKey(c.merchant) === merKey(s.merchant);
        if (fx) return sameMer;                                  // 해외건은 금액이 달라 가맹점으로 대조
        const sameAmt = +c.amount === +s.amount;
        if (!sameAmt) return false;                              // 금액이 다르면 다른 결제
        return sameMer || c.tx_date === s.tx_date;               // 금액 + (가맹점 또는 같은 날)
      });
  ...
  function drawFilter(slot, key, pool, rerender) {
    ...
    const upd = () => { const d = el.querySelector('details'); if (d) d.open = true; rerender(); };
  ...
  async function enter(u) {
    USER = u;
    view = localStorage.getItem('bb_start_view') || 'budget';   // 로그인 후 첫 화면
    ...
    try { await reload(); }
    catch (e) { console.error('불러오기', e); showLoadError([`불러오기: ${e.message || e}`]); }
    go(view);
  }
  ...
  async function reload() {
    ...
    if (!FIXED.length && isAdmin() && window.__SEED__?.fixed) {
      try { await S.insertFixed(window.__SEED__.fixed); FIXED = await S.listFixed(); }
      catch (e) { console.error('고정비 시드', e); }
    }
    ...
    if (!CP.length && isAdmin() && window.__COUPANG__?.length) {
      try { await S.insertCoupang(window.__COUPANG__); CP = await S.listCoupang(); }
      catch (e) { console.error('쿠팡 시드', e); }
    }
    ...
    try { showLoadError(loadErr.concat(RENDER_ERR)); } catch (e) { console.error('오류 표시', e); }
  }
  ...
  function go(v) {
    ...
    if (v === 'prefs') {
      const cur = localStorage.getItem('bb_start_view') || 'budget';
      $$('#startSeg button').forEach(x => x.classList.toggle('on', x.dataset.v === cur));
    }
    ...
    window.scrollTo(0, 0);
    try { renderAll(); } catch (e) { RENDER_ERR.push(`화면 그리기: ${e.message || e}`); }
    try { showLoadError(RENDER_ERR); } catch (e) { console.error(e); }
  }
  ...
  const srcLabel = s => ({ samsung_card: '삼성카드', bank: '기업은행', sheet: '가계부 시트',
                           manual: '직접 입력', sms: '문자 자동', hyundai_card: '현대카드',
                           kb_card: 'KB국민카드' }[s] || s);
  ...
  $('#startSeg')?.addEventListener('click', e => {
    const b = e.target.closest('button'); if (!b) return;
    $$('#startSeg button').forEach(x => x.classList.toggle('on', x === b));
    localStorage.setItem('bb_start_view', b.dataset.v);
    toast(`시작 화면을 '${VIEW_TITLE[b.dataset.v]}' 로 저장했습니다`);
  });
  ...
  window.addEventListener('error', e => {
    try { showLoadError([`오류: ${e.message} (${(e.filename || '').split('/').pop()}:${e.lineno})`]); } catch (x) {}
  });
  window.addEventListener('unhandledrejection', e => {
    try { showLoadError([`오류: ${e.reason && (e.reason.message || e.reason)}`]); } catch (x) {}
  });
